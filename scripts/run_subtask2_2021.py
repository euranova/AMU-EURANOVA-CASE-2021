...
def main(data_dir, epochs, learning_rate, train_batch_size, eval_batch_size, model_name):
    results_dir = Path("results/2021_subtask_2")
    ...
    train_en_texts, train_en_labels = loader.read_json(data_dir / "subtask2-sentence/en-train.json", txt_name="sentence")
    train_es_texts, train_es_labels = loader.read_json(data_dir / "subtask2-sentence/es-train.json", txt_name="sentence")
    train_pt_texts, train_pt_labels = loader.read_json(data_dir / "subtask2-sentence/pr-train.json", txt_name="sentence")
    ...
    with open(data_dir / "english/subtask2-Sentence/test.json", "r") as data_file:
        data = data_file.read()
    test_data_en = [json.loads(elem) for elem in data.split('\n') if elem != ""]
    with open(data_dir / "portuguese/subtask2-Sentence/test.json", "r") as data_file:
        data = data_file.read()
    test_data_pt = [json.loads(elem) for elem in data.split('\n') if elem != ""]
    with open(data_dir / "spanish/subtask2-Sentence/test.json", "r") as data_file:
        data = data_file.read()
    ...
    training_arguments = TrainingArguments(output_dir=results_dir, num_train_epochs=epochs, learning_rate=learning_rate, 
                                           ...
                                           load_best_model_at_end=True, greater_is_better=True, eval_steps=500,
                                           evaluation_strategy="steps", logging_strategy="steps", save_total_limit=3,
                                          )
    ...
    test_texts_en = [test_doc['sentence'] for test_doc in test_data_en]
    test_texts_pt = [test_doc['sentence'] for test_doc in test_data_pt]
    test_texts_es = [test_doc['sentence'] for test_doc in test_data_es]
